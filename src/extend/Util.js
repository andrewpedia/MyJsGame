...
ttutil.getQRCodeUrl = function (url) {
    ...
    if (!url.startsWith("http://") && !url.startsWith("https://")) {
        url = "https://" + apiDomain + "/" + url;
    }

    if (cc.game.config.debugMode == 0) {
        if (cc.sys.os == cc.sys.OS_IOS)
            url = "http://" + apiDomain + "/qrcode?url=" + url;
        else
            url = "http://" + apiDomain + "/qrcode?url=" + url;
    }
    else {
...
if(splitChar==null) return moneyValue;
    var oldMoneyValue = moneyValue;
    moneyValue = moneyValue < 0 ? 0 - moneyValue : moneyValue;

    var str = String(parseFloat(moneyValue).toFixed(2));
    var result = "";
...
ttutil.buildHeadIcon = function (faceID) {
    ...
    var iconBox = new cc.Sprite("#head_frame_box_big.png").to(widget);
...
ttutil.getHeadIconName = function (faceID) {
    return "#head_big_animal_" + (faceID + 1) + ".png.png";
};
...
//浮点精度
ttutil.roundFloat = function (x, n = 2) {
